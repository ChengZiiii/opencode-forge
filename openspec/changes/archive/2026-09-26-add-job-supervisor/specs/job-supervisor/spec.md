# job-supervisor Specification (delta)
...
Each job SHALL be owned by the session that created it. When an owning session ends, its live jobs SHALL be terminated unless ownership was handed off. Plugin `dispose` SHALL terminate all live jobs. Jobs still alive after their owner ended, and completions never read by any session, SHALL be reported in the diagnostics ledger. When the plugin is wholly disabled via the agent disable knob, no job tools SHALL be registered and no job state SHALL persist.
